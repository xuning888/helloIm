...
import com.github.xuning888.helloim.contract.util.DateUtils;
import com.github.xuning888.helloim.store.api.DubboMsgStoreServiceTriple;
...
/**
 * @author xuning
 * @date 2025/11/2 19:01
 */
@DubboService
public class MsgStoreServiceTriple extends DubboMsgStoreServiceTriple.MsgStoreServiceImplBase {

    private static final Logger logger = LoggerFactory.getLogger(MsgStoreServiceTriple.class);
    ...
    public Store.SaveMessageResponse saveMessage(Store.SaveMessageRequest request) {
        Store.ChatMessage chatMessage = request.getChatMessage();
        ...
        if (ChatType.C2C.match(chatMessage.getChatType())) {
            ...
            imMessage.setSendTime(DateUtils.toDate(chatMessage.getSendTime()));
            ...
            imMessageGroup.setSendTime(DateUtils.toDate(chatMessage.getSendTime()));
        ...
        return Store.SaveMessageResponse.newBuilder().setRaw(raw).build();
    }
    ...
    public Store.MaxServerSeqResponse maxServerSeq(Store.MaxServerSeqRequest request) {
        ...
        logger.info("maxServerSeq from: {}, to: {}, chatType: {}, serverSeq: {}, traceId: {}", from, to, chatType, serverSeq, traceId);
        return Store.MaxServerSeqResponse.newBuilder().setServerSeq(serverSeq).build();
    }
}
